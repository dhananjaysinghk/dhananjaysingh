...
import { Menu, X, Sun, Moon, Laptop, Monitor } from "lucide-react"
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/60 backdrop-blur-md transition-all duration-300">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          {/* Actions: Theme Switcher & Mobile Menu Button */}
          <div className="flex items-center space-x-4">
            {/* Theme Toggle Button */}
            {mounted && (
              <button
                onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")}
                className="relative rounded-full p-2 text-muted-foreground hover:bg-muted hover:text-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 transition-all"
            ...
            <button
              ...
              className="flex rounded-full p-2 text-muted-foreground hover:bg-muted hover:text-foreground md:hidden focus:outline-none focus:ring-2 focus:ring-ring"
